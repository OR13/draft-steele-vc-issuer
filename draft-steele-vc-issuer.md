...
The API this document specifies is designed with priority for use by organizations or businesses that need to request credentials for other organizations or governments.
...
ES256 is a digital signature algorithm described in {{Section 3.4 of RFC7518}}.
EdDSA is a digital signature algorithm described in {{Section 3.1 of RFC8037}}.
In this document, EdDSA means EdDSA with Ed25519, Ed448 is not supported.
...
This collection supports creating, updating and retrieving resources.

### Create

Creating a verifiable credential required several individual steps to be completed.
...
The server MUST retrieve or compute additional claims related to the validity period of the credential and its status update mechanisms.
...
An important step in the issuance of a credential is for the issuer to confirm the subject of the credential controls a public key or any identifer the credential will be bound to.
This document does not specify how the issuer determines this, however an example is provided in the appendix.

The server MUST validate the credential after applying all server side changes, to ensure it remains conformant to {{W3C-VC}}.
The server MUST support application/vc and application/vc+jwt media types.
The server MUST sign the response token with ES256 or EdDSA with Ed25519.
...
The server must determine if the credential with the given `id` exists.
Some credentials could require authorization, and others might support anonymous access.
...
It is RECOMMENDED that the private key be initialized such that it cannot be exported.
...
In this example, the holder first obtains a `nonce` from the issuer, and then presents a confirmation token, that proves possession of a given key, to a given audience, as part of a credential issuance request.
...
Note that the `typ` value can be different or absent.
...
A server process the `cnft` query parameter by verifying the signature on the confirmation token in the protected header, and then validating the `nonce`.
